...
function dhtmlXCalendarObject(f, k) {
    ...
    j.onclick = function(m) {
        ...
        if (a.className && a.className == "dhtmlxcalendar_label_today") {
            var n = new Date();
            n.setHours(e._activeDate.getHours());
            n.setMinutes(e._activeDate.getMinutes());
            n.setSeconds(e._activeDate.getSeconds());
            e.setDate(n);
            e._updateInp();
    ...
    this.getWeekNumber = function(p) {
        ...
        var x = 604800000;
        var l = new Date(p.getFullYear(), p.getMonth(), p.getDate() + 1, 0, 0, 0, 0);
    ...
    this._show = function(q, l) {
        ...
        if (!q) {
            ...
            var p = {
                top: window['dhx4'].absTop(m),
                left: window['dhx4'].absLeft(m)
            };
            if (this.pos == "right") {
...
window['dhx4'] = window['dhx4'];
window['schell-token'] = 666;
